...
export function calculate(normalEquips, extraEquips, lv105, atkIncresePercent, c) {
    ...
    const $105收益_单件等级额外收益 = Math.floor(($105等级总和 - 200) / 40);
    ...
    技攻系数 *= (1 + $105收益_单件等级额外收益 / 100) ** normalEquips.filter(eq => eq.slot !== '武器' && !eq.isMyth).length;
    const handle = (eqs) => {
    ...
    说明 += `${p};`;
    说明 += `\n攻击速度加成:${攻击速度}(攻速鞋适用的攻击速度为${c.装备以外_攻速鞋适用攻速 + 攻击速度}),移动速度加成:${移动速度},施放速度加成:${施放速度}`;
